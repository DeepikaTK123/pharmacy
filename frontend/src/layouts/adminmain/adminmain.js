...
const AdminMain = () => {
  ...
  const fetchSolutionList = () => {
    ...
    axios
      .get(
        solutiontypes(userProfile?.attributes?.tenant_id[0])
      )
      .then((response) => {
  ...
  const sendToBackend = (fileName, solutionJsonData, deviceJsonData) => {
    ...
    formData.append("tenantId", userProfile?.attributes?.tenant_id[0]);
      ...
      {(solutionList?.length ?? 0) > 0 && (
        <div >
          <Heading color={textColor}
            fontSize='22px'
            fontWeight='700'
            lineHeight='100%' mb={2}>List of Dashboard:</Heading>
